...
public class MyLobbyManager : MonoBehaviour
    {
        ...
        {
            get => HasLobby && HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyMemberData(Lobby, HeathenEngineering.SteamworksIntegration.API.User.Client.Id, LobbyData.DataReady) == "true";
            set => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.SetLobbyMemberData(Lobby, LobbyData.DataReady, value.ToString().ToLower());
        }
        /// <summary>
        ...
        /// created by Heathen's tools such as <see cref="LobbyManager"/> and of course <see cref="HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client"/>
        /// </summary>
        ...
        {
            get => HasLobby ? HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyMemberLimit(new CSteamID(Lobby)) : 0;
            set => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.SetLobbyMemberLimit(new CSteamID(Lobby), value);
        }
        /// <summary>
        ...
        /// </summary>
        public LobbyGameServer GameServer => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyGameServer(Lobby);
        ...
        {
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyAskedToLeave.AddListener(HandleAskedToLeave);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyDataUpdate.AddListener(HandleLobbyDataUpdate);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyLeave.AddListener(HandleLobbyLeave);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyGameCreated.AddListener(HandleGameServerSet);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyChatUpdate.AddListener(HandleChatUpdate);
        }

        private void OnDisable()
        {
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyAskedToLeave.RemoveListener(HandleAskedToLeave);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyDataUpdate.RemoveListener(HandleLobbyDataUpdate);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyLeave.RemoveListener(HandleLobbyLeave);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyGameCreated.RemoveListener(HandleGameServerSet);
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.EventLobbyChatUpdate.RemoveListener(HandleChatUpdate);
        }
        ...
        public bool SetType(ELobbyType type)
        {
            ...
            return HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.SetLobbyType(Lobby, type);
        }
        /// <summary>
        ...
        /// <returns></returns>
        public bool SetJoinable(bool makeJoinable) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.SetLobbyJoinable(Lobby, makeJoinable);
        /// <summary>
        ...
        public void QuickMatch(bool createOnFail = true)
        {
            ...
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListDistanceFilter(searchArguments.distance);

            if (searchArguments.slots > 0)
                HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListFilterSlotsAvailable(searchArguments.slots);

            foreach (var near in searchArguments.nearValues)
                HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListNearValueFilter(near.key, near.value);

            foreach (var numeric in searchArguments.numericFilters)
                HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListNumericalFilter(numeric.key, numeric.value, numeric.comparison);

            foreach (var text in searchArguments.stringFilters)
                HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListStringFilter(text.key, text.value, text.comparison);

            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListResultCountFilter(1);

            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.RequestLobbyList((r, e) =>
            {
                if (!e && r.Length >= 1)
                {
                    HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.JoinLobby(r[0], (r2, e2) =>
                    {
                        var responce = r2.Response;

                        if (!e2 && responce == EChatRoomEnterResponse.k_EChatRoomEnterResponseSuccess)
                        {
                            if (HeathenEngineering.SteamworksIntegration.API.App.isDebugging)
        ...
        public void Create(Action<EResult, LobbyData, bool> callback)
        {
            ...
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.CreateLobby(createArguments.type, createArguments.slots, (result, lobby, ioError) =>
            {
                if (!ioError)
                {
        ...
        public void Search(int maxResults)
        {
            ...
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.AddRequestLobbyListResultCountFilter(maxResults);
        ...
        public void Join(LobbyData lobby)
        {
            ...
            HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.JoinLobby(lobby, (r, e) =>
            {
                if (!e)
                {
        ...
        public bool SetLobbyData(string key, string value) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.SetLobbyData(Lobby, key, value);
        public void SetMemberData(string key, string value) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.SetLobbyMemberData(Lobby, key, value);
        public LobbyMemberData GetLobbyMember(UserData member) => new LobbyMemberData { lobby = Lobby, user = member };
        public string GetLobbyData(string key) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyData(Lobby, key);
        public string GetMemberData(UserData member, string key) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyMemberData(Lobby, member, key);
        public bool IsMemberReady(UserData member) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyMemberData(Lobby, member, LobbyData.DataReady) == "true";
        public void KickMember(UserData member) => Lobby.KickMember(member);
        public void Invite(UserData user) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.InviteUserToLobby(Lobby, user);
        public void Invite(uint FriendId) => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.InviteUserToLobby(Lobby, UserData.Get(FriendId));
        public LobbyMemberData[] Members => HeathenEngineering.SteamworksIntegration.API.Matchmaking.Client.GetLobbyMembers(Lobby);
    }
}
